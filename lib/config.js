...
async function loadConfig(configLocation, processed = []) {
  ...
  if (isAbsoluteURL(configLocation)) {
    ...
    const st = await fs.promises.stat(configLocation)
    if (st.isDirectory()) {
      // try to find a repolinter config file in directory
      configLocation =
        findConfig('repolint.json', { cwd: configLocation }) ||
        findConfig('repolint.yaml', { cwd: configLocation }) ||
        findConfig('repolint.yml', { cwd: configLocation }) ||
        findConfig('repolinter.json', { cwd: configLocation }) ||
        findConfig('repolinter.yaml', { cwd: configLocation }) ||
        findConfig('repolinter.yml', { cwd: configLocation }) ||
